  ...
  if (!focusArticle) {
    return (
      <div className="card text-sm text-slate-500">
        Select a headline and choose <span className="font-semibold">Ask ChatGPT</span> to explore it.
      </div>
    );
  }

  return (
    <div className="card flex h-[380px] flex-col border border-slate-200/80 bg-white/90 shadow-md backdrop-blur dark:border-slate-800/80 dark:bg-slate-900/90">
      <header className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3 dark:border-slate-800/70">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
            ...
            {focusArticle.url ? (
              <a
                ...
                className="inline-flex items-center gap-1 text-status-green underline-offset-4 hover:underline"
              >
                View article
              </a>
            ) : null}
        ...
        <button
          ...
          className="rounded-md border border-slate-200 px-2 py-1 text-xs font-medium text-slate-500 transition hover:border-status-green hover:text-status-green dark:border-slate-700 dark:text-slate-400"
        >
          Close
        </button>
      </header>
      ...
      <div
        ref={scrollRef}
        className="custom-scrollbar relative mt-3 flex-1 space-y-3 overflow-auto pr-1"
      >
          ...
          <div
            ...
            className={`rounded-lg px-3 py-2 text-sm leading-relaxed ${
              message.role === 'assistant'
                ...
                : 'bg-status-green/15 text-slate-800 dark:bg-status-green/25 dark:text-slate-100'
            }`}
          >
        ...
        {isLoading ? (
          <div className="flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-500 dark:bg-slate-800 dark:text-slate-300">
      ...
      {error ? (
        <div className="mt-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600 dark:border-red-400/40 dark:bg-red-950/40 dark:text-red-200">
          {error}
        </div>
      ) : null}
      ...
      <form onSubmit={handleSubmit} className="mt-3 flex flex-col gap-2">
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="custom-scrollbar flex-1 resize-none rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm transition focus:border-status-green focus:outline-none focus:ring-2 focus:ring-status-green/40 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
            disabled={isLoading || !OPENAI_CONFIGURED}
          />
          <button
            type="submit"
            disabled={isLoading || !input.trim() || !OPENAI_CONFIGURED}
            className="inline-flex items-center justify-center rounded-md bg-status-green px-3 py-2 text-sm font-semibold text-slate-900 shadow transition hover:bg-status-green/90 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
          >
            Send
          </button>
        </div>
